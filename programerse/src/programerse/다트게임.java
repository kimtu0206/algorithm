...
public class 다트게임 {
	...
	class Solution {
		public int solution(String dartResult) {
			...
			String temp = ""; // 문자열 내 1번 점수 저장 변수
			int arr[] = new int[3]; // 각 단계별 획득 점수를 저장할 배열
			...
			for (int i = 0; i < dartResult.length(); i++) {
				...
				// 1-2 * 인 경우는 자신과 이전의 점수 *2, idx는 S,D,T 처리후 증가하기 때문에 idx-1, idx-2 후 *2
				case '*':
					arr[idx - 1] *= 2;
					if (idx > 1)
						arr[idx - 2] *= 2;
				...
				// 1-7 그 외의 정수들을 temp 변수에 저장 문자가 없을 수도 있기 때문에 valueOf를 사용
				default:
			...
			// 위의 과정을 통해 구한 각 횟수의 점수를 모두 더하면 정답
